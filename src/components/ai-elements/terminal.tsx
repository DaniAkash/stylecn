...
// ansi-to-react is a CJS module marked __esModule. Vite wraps it again on a
// namespace import, so the real component lands at AnsiModule.default.default.
// Walk down until we hit the function so the JSX <Ansi /> below works.
import * as AnsiModule from "ansi-to-react";
type AnsiComponent = React.ComponentType<{ children: string }>;
function resolveAnsi(mod: unknown): AnsiComponent {
  let current: unknown = mod;
  for (let i = 0; i < 3; i++) {
    if (typeof current === "function") return current as AnsiComponent;
  ...
  throw new Error("ansi-to-react: could not resolve default export");
}
